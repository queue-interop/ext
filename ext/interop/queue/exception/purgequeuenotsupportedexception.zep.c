...
ZEPHIR_INIT_CLASS(Interop_Queue_Exception_PurgeQueueNotSupportedException) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Interop\\Queue\\Exception, PurgeQueueNotSupportedException, interop, queue_exception_purgequeuenotsupportedexception, interop_queue_exception_exception_ce, interop_queue_exception_purgequeuenotsupportedexception_method_entry, 0);
...
PHP_METHOD(Interop_Queue_Exception_PurgeQueueNotSupportedException, providerDoestNotSupportIt) {
	...
	object_init_ex(return_value, interop_queue_exception_purgequeuenotsupportedexception_ce);
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "The provider does not support purge queue.");
	ZVAL_LONG(&_1, code);
