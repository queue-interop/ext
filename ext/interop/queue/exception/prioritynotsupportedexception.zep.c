...
ZEPHIR_INIT_CLASS(Interop_Queue_Exception_PriorityNotSupportedException) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Interop\\Queue\\Exception, PriorityNotSupportedException, interop, queue_exception_prioritynotsupportedexception, interop_queue_exception_exception_ce, interop_queue_exception_prioritynotsupportedexception_method_entry, 0);
...
PHP_METHOD(Interop_Queue_Exception_PriorityNotSupportedException, providerDoestNotSupportIt) {
	...
	object_init_ex(return_value, interop_queue_exception_prioritynotsupportedexception_ce);
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "The provider does not support priority feature");
	ZVAL_LONG(&_1, code);
